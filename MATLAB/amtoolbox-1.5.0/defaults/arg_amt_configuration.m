...
%
%   Url: http://amtoolbox.org/amt-1.5.0/doc/defaults/arg_amt_configuration.php


%   #License: GPL
%   #Author: Clara Hollomey (2021)
%   #Author: Piotr Majdak (2023): cleaned up, adapted to faster amt_start
...
definput.keyvals.pythoncmd = ''; % empty string = no python available
definput.keyvals.python = 'not determined'; % Python version, if available
...
definput.keyvals.sofa = num2str('https://sourceforge.net/projects/sofacoustics/files/SOFA%20Toolbox%20for%20Matlab%20and%20Octave%202.2.0.zip/download');
...
fid=fopen(fullfile(definput.keyvals.path,'amt_version')); x=fread(fid); fclose(fid);
v=['amt-' char(x(1:end-2)')];
definput.keyvals.version = {v, 'amt-1.4.0', 'amt-1.3.0', 'amt-1.2.0', 'amt-1.1.0', 'amt-1.0.0'};
...
definput.keyvals.auxdataPath = num2str([definput.keyvals.path, 'auxdata']);
